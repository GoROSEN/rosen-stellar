...
func (c *Controller) signUp(ctx *gin.Context) {
	...
	// post signup
	if err := c.postSignupV2(obj); err != nil {
...
func (c *Controller) postSignupV1(obj *member.Member) error {
...
func (c *Controller) postSignupV2(obj *member.Member) error {
	...
	// 创建token账户
	// c.accountService.NewAccount(obj.ID, obj.UserName, tokenName)
	c.accountService.NewAccount(obj.ID, obj.UserName, cfg.Rosen.Coin.TokenName)
	// 创建金币账户
	c.accountService.NewAccount(obj.ID, obj.UserName, cfg.Rosen.Coin2.TokenName)
	...
	if err == nil {
		...
		if isNew {
			if err := c.service.SetupNewMember(c.service.Db, m); err == nil {
				// post signup
				if err = c.postSignupV2(m); err != nil {
					log.Errorf("post signup error: %v", err)
				}
			}
		}
